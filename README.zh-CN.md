...
一款基于微信小游戏 Canvas 的五行策略卡牌原型。玩家通过选择符牌、触发灵猫效果、积累灵力池并攻击 Boss，在有限出牌和弃牌次数内完成 3 个世界、9 个关卡的挑战。

当前版本的核心已经从早期“扑克牌型改造”迭代为更独立的五行战斗系统：`金 / 木 / 水 / 火 / 土` 不再只是花色替代，而是直接决定出牌组合、灵猫效果、Boss 克制关系和局内成长。

## 项目亮点

- **原创战斗系统**：将 `金 / 木 / 水 / 火 / 土` 设计为完整的出牌组合、成长、克制和 Boss 结算体系，而不是简单替换传统扑克牌花色。
- **移动端 Canvas 交互**：实现竖屏布局、触控选牌、战斗结算、动画反馈、血条护盾和局内状态展示。
- **可配置规则模块**：将元素、牌型、关卡、Boss 和商店摆件等内容拆分到配置与逻辑模块中，方便后续平衡和扩展。
- **完整原型流程**：包含开始页、战斗页、商店页、通关/失败状态、牌库预览、牌型提示和结算反馈。
- **适合展示能力**：项目覆盖游戏规则设计、状态管理、Canvas 渲染、移动端适配、交互反馈和模块化 JavaScript 实现。
